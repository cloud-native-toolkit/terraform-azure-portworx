...
metadata:
  ...
  namespace: kube-system
data:
  px-essen-user-id: ${data.external.portworx_config.result.user_id}
  px-osb-endpoint: ${data.external.portworx_config.result.osb_endpoint}
%{endif}
EOF
}
...
parameters:
  repl: "1"%{if local.px_enterprise && var.enable_encryption}${indent(2, "\nsecure: \"true\"")}%{endif}
  priority_io: "high"
  sharedv4: "true"
...
provisioner: kubernetes.io/portworx-volume
parameters:
  repl: "3"%{if local.px_enterprise && var.enable_encryption}${indent(2, "\nsecure: \"true\"")}%{endif}
  priority_io: "high"
...
parameters:
  priority_io: high
  repl: "2"%{if local.px_enterprise && var.enable_encryption}${indent(2, "\nsecure: \"true\"")}%{endif}
  sharedv4: "true"
  io_profile: "db_remote"
  disable_io_profile_protection: "1"
allowVolumeExpansion: true
